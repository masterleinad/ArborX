 ...
 * Since we are using two different queries, we need to AccessTraits
 * specializations.
 */
...
template <typename MemorySpace>
struct ArborX::AccessTraits<RaysFirst<MemorySpace>, ArborX::PredicatesTag>
{
  using memory_space = MemorySpace;
  using size_type = std::size_t;

  KOKKOS_FUNCTION
  static size_type size(RaysFirst<MemorySpace> const &rays)
  {
    return rays._rays.extent(0);
  }
  KOKKOS_FUNCTION
  static auto get(RaysFirst<MemorySpace> const &rays, size_type i)
  {
    return attach(ordered_nearest(rays._rays(i)), (int)i);
  }
};
...
template <typename MemorySpace>
struct ArborX::AccessTraits<RaysSecond<MemorySpace>, ArborX::PredicatesTag>
{
  using memory_space = MemorySpace;
  using size_type = std::size_t;

  KOKKOS_FUNCTION
  static size_type size(RaysSecond<MemorySpace> const &rays)
  {
    return rays._rays.extent(0);
  }
  KOKKOS_FUNCTION
  static auto get(RaysSecond<MemorySpace> const &rays, size_type i)
  {
...
/*
 * The DepositEnergy struct is used in the first approach to directly deposit
 * energy.
 */
template <typename MemorySpace>
...
int main(int argc, char *argv[])
{
  ...
  bvh.query(exec_space, RaysFirst<MemorySpace>{rays},
  ...
  Kokkos::parallel_reduce(
      ...
      KOKKOS_LAMBDA(int i, int &error) {
        ...
        float const rel_error =
            (energy(i) == 0.f)
                ? 0.f
                : fabs(my_energy(i) - energy(i)) / fabs(energy(i));
