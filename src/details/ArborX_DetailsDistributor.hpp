                                         ...
                                         std::vector<int> &offsets)
{
  ...
  ARBORX_ASSERT(permutation_indices.extent_int(0) == lastElement(batched_offsets));
  static_assert(
  ...
  Kokkos::View<int *, DeviceType> device_batched_ranks_duplicate(
      Kokkos::ViewAllocateWithoutInitializing(batched_ranks.label()), batched_ranks.size());
    ...
    Kokkos::parallel_scan(ARBORX_MARK_REGION("process_biggest_rank_items"),
                          ...
                          KOKKOS_LAMBDA(int i, Result &update, bool last_pass) {
                            bool const is_largest_rank =
                                (device_batched_ranks_duplicate(i) == largest_rank);
                            if (is_largest_rank)
                            {
                              if (last_pass)
                              {
                                device_batched_permutation_indices(i) = update.batch_count + batch_offset;
  ...
  Kokkos::View<int *, DeviceType> device_permutation_indices(Kokkos::ViewAllocateWithoutInitializing("batched_permutation_indices"), permutation_indices.size());
...
public:
  ...
  size_t createFromSends(View const &batched_destination_ranks, View const& batch_offsets)
  {
    ...
    sortAndDetermineBufferLayout(batched_destination_ranks, batch_offsets, _permute, _destinations,
