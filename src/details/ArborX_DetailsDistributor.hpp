...
public:
                       ...
                       size_t num_packets, View const &imports) const
  {
    ...
    for (int i = 0; i < outdegrees; ++i)
    {
      ...
      {
        auto const it =
            std::find(_sources.begin(), _sources.end(), comm_rank);
        ARBORX_ASSERT(it != _sources.end());
	auto const position = it-_sources.begin();
	assert(position < _src_offsets.size());
	assert(_src_offsets[position]*num_packets < src_buffer.size());
        assert(i < _dest_offsets.size());
        assert(_dest_offsets[i]*num_packets < dest_buffer.size());
	assert(_dest_offsets[i]*num_packets+_dest_counts[i] * num_packets <= dest_buffer.size());
        assert(_src_offsets[position]*num_packets+_dest_counts[i] * num_packets <= src_buffer.size());
        std::memcpy(
